 ...
 */
 ...
 rl.question(
   'Drag here your exonic amplicon data (file created by the UCSC Genome Browser tools) with the format described in the README file \n',
   (ampliconPath) => {
     rl.question(
       'Drag here the cDNA coordinates of the reference exome data with the format described in the README file \n',
       (exonPath) => {
         rl.question(
           ...
           (finalFileName) => {
             main(ampliconPath, exonPath, finalFileName);
           }
         );
       }
     );
 ...
 function main(ampliconPath, exonPath, finalFileName) {
   ...
   const currentAmpliconHeaders = amplicons[0]
     .toLowerCase()
     .trim()
     .replace(/\s/g, '_')
     .replace(/\./g, '_')
   ...
   let exonRawData = fs.readFileSync(exonPath.replace(/\"/g, ''));
   ...
   const headers =
     'amplicon.name;gene.symbol;accession.number;exon.number;chromosome;startDNA;endDNA;strand;startcDNA;endcDNA;startcodon;endcodon';
   ...
   amplicons = amplicons.map((amplicon) =>
     convertIntoObject(currentAmpliconHeaders, amplicon.split(';'))
   );
 
   exons = exons.map((exon) =>
     convertIntoObject(exonsCurrentHeaders, exon.split(';'))
   );
   ...
   amplicons.map((amplicon) => {
     ...
     let gen =
       groupByExon.filter((gene) => amplicon.gene_symbol === gene[0].gene) || [];
     //take only the exon that is containing the amplicon
     let exon = gen[0]?.filter(
       (gen) => gen.startdna <= amplicon.start && gen.enddna >= amplicon.end
     );
 ...
 function convertIntoObject(headerArray, dataArray) {
   ...
   for (let position = 0; position < headerArray.length; position++) {
     ...
     if (
       !isNaN(parseInt(dataArray[position])) &&
       (dataArray[position].match(/,/g) || []).length < 2
     ) {
