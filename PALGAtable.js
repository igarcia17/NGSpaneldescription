...
const updatedHeaders = ['gene_name', 'accession_number', 'exons_number', 'cDNA_coordinates', 'codons_covered'];
...
let geneObjects = rawData.map( gene => {
    ...
    for ( let position = 0; position < currentHeaders.length; position++ )
    {
        ...
        geneObject[currentHeaders[position]] = geneData[position].replace( /\s/g, '' );
...
groupByGen.map( geneArray => {
    ...
    let finalGeneArray = [geneArray[0].gene_symbol, geneArray[0].accession_number];
...
fs.writeFile( 'tableOCA.csv', finalData, function ( err ) {
...
function mergeExonNumbers ( exonNumberArray ) {
    ...
    let swaps = 0;
    console.log( prrocessedExonNumbers );
    for ( let position = 0; position < exonNumberArray.length - 1; position++ )
    {
        ...
        {
            firstExonNumberToCompare = parseInt( prrocessedExonNumbers[position].split( '-' )[1] );
            firstExonNumberToReplace = parseInt( prrocessedExonNumbers[position].split( '-' )[0] );
        } else
        {
        ...
        {
            secondExonNumberToCompare = parseInt( prrocessedExonNumbers[position + 1].split( '-' )[0] );
            secondExonNumberToReplace = parseInt( prrocessedExonNumbers[position + 1].split( '-' )[1] );
        } else
        {
            secondExonNumberToCompare = prrocessedExonNumbers[position + 1];
        }
        console.log( 'a comparar:', firstExonNumberToCompare, secondExonNumberToCompare );
        if ( secondExonNumberToCompare - firstExonNumberToCompare <= 1 )
        {

            const firstCoordinate = firstExonNumberToReplace ? firstExonNumberToReplace : firstExonNumberToCompare;
            const secondCoordinate = secondExonNumberToReplace ? secondExonNumberToReplace : secondExonNumberToCompare;
            const newCoordinates = firstCoordinate !== secondCoordinate ? `${ firstCoordinate }-${ secondCoordinate }` : firstCoordinate;
...
function mergeCoordinates ( coordinatesArray ) {
    ...
    for ( let position = 0; position < processedCoordinates.length - 1; position++ )
    {
...
function sortByExon ( gene ) {
    ...
    {
        if ( gene[position].exon_number > gene[position + 1].exon_number ||
